            ...
            <div class="col-lg-8">
                ...
                <form action="{{ route('categories.update', $category) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <small class="text-muted">Ej: Tecnología, Deportes, Música, etc.</small>
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex align-items-center gap-3">
                            <input type="color"
                                ...
                                id="color_picker" name="color" value="{{ old('color', $category->color) }}"
                                style="width: 60px; height: 38px; cursor: pointer;">
                            <input type="text" class="form-control @error('color') is-invalid @enderror" id="color_text"
                                value="{{ old('color', $category->color) }}" placeholder="#RRGGBB" style="flex: 1;">
                        </div>
                        <small class="text-muted">Select a representative color for the category</small>
                        @error('color')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-check form-switch">
                            ...
                            <label class="form-check-label ms-2" for="is_active" id="status_label">
                                {{ old('is_active', $category->is_active) == '1' ? 'Activo' : 'Inactivo' }}
                            </label>
                        </div>
                    ...
                    <div class="alert alert-light border mb-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <small>{{ $category->created_at ? $category->created_at->format('d/m/Y H:i') : 'N/A' }}</small>
                            </div>
                            <div class="text-end">
                                <small class="text-muted d-block">Last update:</small>
                                <small>{{ $category->updated_at ? $category->updated_at->format('d/m/Y H:i') : 'N/A' }}</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex align-items-center gap-3 p-3 rounded" id="previewCard">
                            <div class="rounded-2 d-flex align-items-center justify-content-center text-white fw-bold"
                                style="width:50px; height:50px; background-color: {{ $category->color }}; font-size:1.2rem;">
                            ...
                            <span class="badge {{ $category->is_active ? 'bg-success' : 'bg-secondary' }} ms-auto"
                                id="previewStatus">
...
@push('scripts')
    <script>
        ...
        nameInput.addEventListener('input', function() {
            ...
            document.getElementById('previewSlug').textContent = slug || 'slug-de-ejemplo';
